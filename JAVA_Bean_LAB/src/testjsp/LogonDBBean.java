...
public class LogonDBBean {
	...
	public int userCheck(String id, String passwd) {
		...
		try {
			...
			pstmt = conn.prepareStatement("select mem_pwd from member2 where mem_id = ?");
	...
	public int confirmId(String id) {
		...
		try {
			conn = getConnection();
			pstmt = conn.prepareStatement("select mem_id from member2 where mem_id = ?");
			pstmt.setString(1, id);
			rs = pstmt.executeQuery();
			if (rs.next())// 아이디 존재
				x = 1; // 같은 아이디 있음
	...
	public boolean insertMember(LogonDataBean member) {
		...
		try {
			...
			pstmt = conn.prepareStatement("insert into member2 values (null,?,?,?,?,?,?)");
	...
	public LogonDataBean getMember(String id, String passwd) {
		...
		try {
			conn = getConnection();
			String orgPass = passwd;
			pstmt = conn.prepareStatement("select * from member2 where mem_id = ?");
			pstmt.setString(1, id);
			rs = pstmt.executeQuery();
	...
	public int updateMember(LogonDataBean member) {
		...
		try {
			...
			if (rs.next()) {// 해당 아이디가 있으면 수행
				String dbpasswd = rs.getString("mem_pwd");
				if (dbpasswd.equals(orgPass)) {
					pstmt = conn.prepareStatement("update member2 set mem_name=?,address=?,tel=? " + "where mem_id=?");
	...
	public int deleteMember(String id, String passwd) {
		...
		try {
			conn = getConnection();
			String orgPass = passwd;
			pstmt = conn.prepareStatement("select mem_pwd from member2 where id = ?");
			pstmt.setString(1, id);
			rs = pstmt.executeQuery();
			if (rs.next()) {
				String dbpasswd = rs.getString("mem_pwd");
				if (dbpasswd.equals(orgPass)) {
					pstmt = conn.prepareStatement("delete from member2 where mem_id=?");
					pstmt.setString(1, id);
					pstmt.executeUpdate();
					x = 1;// 회원탈퇴처리 성공
				} else
